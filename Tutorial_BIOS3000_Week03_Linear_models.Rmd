---
title: Tutorial and Assignment Week 3 - Introduction to linear statistical models
...
In this week's tutorial we will get introduced to linear statistical models and analyse the morphometric data we collected in last week's group work.I suggest you run the code below by either copy-pasting into an empty R script file or by running each chuck in the R Markdown (Rmd-file).

I suggest you create sub-directories "Week01", "Week02", "Week03" and so on somewhere on your hard-drive. Download the Rmd-file and the data-file "MeasurementData.txt" to the "Week03" folder. Open the Rmd-file from this folder and go into "Sessions > Set Working Directory" in the top menu in R Studio and select "To Source File" (you can see the R command that this issues in the Console window). In this way you don't need to write the full path to access or create any files in this directory.
...
students_means = aggregate(students[,6:13], by=list(students$ID), FUN=mean, na.rm = TRUE)
head(students_means)
```
...
Now we have a new data frame `students_means` that only has one value for each person (the mean of the three measurements). We have however lost the variable `Sex`. A trick to retain this variable in the new data frame is to include it as a grouping variable (although it will not refine the groupings because each person only has one sex). Below, we also set the names of the two first variables.

```{r}
students_means = aggregate(students[,6:13], by=list(students$ID, students$Sex), FUN=mean, na.rm = TRUE)
names(students_means)[1:2] = c("ID", "Sex")
head(students_means)
```

Let's also compute the average of the measurements of the left and right foot and store this in a new variable called `Foot` in `students_means`:

```{r}
students_means$Foot = (students_means$Length_of_right_foot + students_means$Length_of_left_foot)/2
```

We want to use these aggregated data in later tutorials, so let's save it to your working directory:

```{r}
write.csv(students_means, "students_means.csv")
```
...
```{r}
plot(students_means$Foot, students_means$Height)
```
...
```{r}
plot(students_means$Height ~ students_means$Foot) # An alternative way of plotting
lin_reg = lm(Height ~ 1 + Foot, data = students_means) # lm() is a function for fitting a linear model with normal residuals
abline(lin_reg, col="blue")
abline(h = mean(students_means$Height), col="red")
```

The linear regression is here done with the `lm()` function and the lines are drawn with the versatile `abline()` function (remember that you can look at the documentation for these functions by typing e.g. `?abline` in the Console window).

Both these lines represent what we call **"statistical models"** of the data. More precisely, these lines are the **predictions** from two different models fitted to the data (using linear regression). Even though we did not fit a model with the `lm()` function to draw the red line here, we would have got the same line if we did. You can confirm this by removing the predictor variable `Foot` from the above model specification (i.e., use `lin_reg = lm(Height ~ 1, data = students_means)`)
...
```{r}
fit_1 = lm(Height ~ 1, data = students_means)
```
...
But, wait! Is this reasonable? We see from the summary of the data above that there are `r table(students_means$Sex)[1]` females and only `r table(students_means$Sex)[2]` males in the data set (i.e., `r  round(table(students_means$Sex)[1]/nrow(students_means), 2)*100` % females). We also know that females are on average shorter than males. Since we have unbalanced data with respect to sex, the mean we have computed above is more influenced by females than males, and this mean is not very meaningful.
...
```{r}
#mean(students_means$Height[students_means$Sex=="female"])
#sd(students_means$Height[students_means$Sex=="female"])

#with(students_means[students_means$Sex=="male",], mean(Height))
#with(students_means[students_means$Sex=="male",], sd(Height))

(means = tapply(students_means$Height, list(students_means$Sex), mean))
(SDs = tapply(students_means$Height, list(students_means$Sex), sd))
```
...
```{r}
(SEs = SDs/sqrt(table(students_means$Sex)))
```
...
```{r}
fit_F = lm(Height ~ 1, data = students_means, subset = Sex=="female")
summary(fit_F)
confint(fit_F)
```

```{r}
fit_M = lm(Height ~ 1, data = students_means, subset = Sex=="male")
summary(fit_M)
confint(fit_M)
```
...
The standard deviation in body height appears to be somewhat lower for males than for females (`r round(sd(students_means$Height[students_means$Sex=="male"]),2)`  vs. `r round(sd(students_means$Height[students_means$Sex=="female"]),2)`). This difference can partly be due to sampling error (you will address this in the Assignment this week. Since they are not hugely different, we may justify fitting a model where residual standard deviation is the same for the two sexes (remember that any model is a simplification of reality, and we, as researchers have to decide how much we want to simplify). We can do this in the `lm()` function:
...
```{r}
fit_Sex_no_int = lm(Height ~ -1 + Sex, data = students_means)
...
Sex = students_means$Sex
n = length(Sex)  # Sample size

samples_beta = matrix(nrow=n_iter, ncol=2)  # Empty matrix for storing resampled beta-estimates
for(i in 1:n_iter){
  fixed = beta[1] + (as.numeric(Sex)-1)*beta[2]  # Another way: fixed = ifelse(fit_Sex$model$Sex=="female", beta[1], sum(beta))
...
for(i in 1:n_iter){
  refit = lm(Y[,i] ~ students_means$Sex)
...
for(i in 1:n_iter){
  ...
  resamp_data = students_means[resamp_rows,]
...
```{r}
fit_Foot = lm(Height ~ Foot, data = students_means)
```
...
```{r}
fit_Sex_Foot = lm(Height ~ Sex + Foot, data = students_means)
...
```{r}
mu_F = mean(students_means$Foot[students_means$Sex=="female"])
fit_Sex_Foot = lm(Height ~ Sex + I(Foot-mu_F), data = students_means)
summary(fit_Sex_Foot)
```
...
x_range_F = range(students_means$Foot[students_means$Sex=="female"])
x_range_M = range(students_means$Foot[students_means$Sex=="male"])
...
with(students_means, points(Foot, Height, col=ifelse(Sex=="female", "red", "blue")))
...
```{r}
var_res = var(residuals(fit_Sex_Foot_interaction))
var_tot = var(residuals(fit_1)) # = var(students_means$Height)
...
```{r}
(sd_res = sd(residuals(fit_Sex_Foot_interaction)))
(sd_res_process = sqrt(sd_res^2 - var_merr_mean))
```

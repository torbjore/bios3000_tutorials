...
<!-- and analyze the morphometric data we collected in last week's group work. -->
I suggest you run the code below by copy-pasting into an empty R script or Markdown file.
...
Take a moment to study this plot and the model summary above, and compare this to the result from the additive model. Note that in this model, $\beta_1$ is the difference between the two sexes *only at the intercept* (when foot length is zero) as we no longer assume that the two prediction lines are parallel. Hence, $\beta_1$ has no biological interpretation (as nobody has foot length of zero) 

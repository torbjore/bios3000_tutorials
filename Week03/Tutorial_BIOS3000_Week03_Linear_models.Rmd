...
In this week's tutorial we will get introduced to linear statistical models, and analyze the morphometric data we collected in last week's group work. I suggest you run the code below by copy-pasting into an empty R script or Markdown file.

I suggest you create sub-directories "Week01", "Week02", "Week03" and so on somewhere on your hard-drive and download data-files 
"MeasurementData_2023.txt" and "MeasurementData_2020.txt" 
<!-- "student_means.csv" -->
to the "Week03" folder. Note that the data file must be in the same folder as any R Mardown (Rmd-file) that will read it.
...
Since there are rather few students this year, we will combine the data with the data from 2020 and 2023 (there were no measurements in 2021 and 2022 due to COVID and the course was not offered in 2024):
...
# all(names(students_2025) == names(students_2023)) # Checking that column names are identical
students = rbind(students_2020, students_2023, students_2025)
# summary(students)
```
...
Here there are three measurement for each individual. Ignoring this fact would clearly be wrong as we would think have three times as many individuals in the data as we actually have (this would be a case of "pseudoreplication" that will get back to later in the course).
...
We have now estimated that the mean height of females in the population is `r round(means[1] ,1)` cm, and for males the mean height is `r round(means[2], 1)` cm. The standard deviation describing the variation among females and males are respectively `r round(SDs[1] ,1)` cm and  `r round(SDs[2], 1)` cm. These estimates are associated with some uncertainty. You may remember from last week that the standard error of the mean is the standard deviation divided by $\sqrt{n}$ (where $n$ is sample size). Using this we get:

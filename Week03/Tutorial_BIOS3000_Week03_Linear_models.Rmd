...
```{r}
summary(students_2023)
```

This shows a summary of the distribution of each of the variables in the data. When importing data, it is important to check the summary to make sure that there are no unexpected NA's (missing values) or inconsistent labeling such as "male" and "male " (note the accidental extra space; I had to fix some issues of this type, but that data looks as expected now).

Since there are rather few students this year, and there are only four males, we will combine the data with the data from 2020 (there were no measurements in 2021 and 2022 due to COVID):
...
all(names(students_2020) == names(students_2023)) # Checking that column names are identical
students = rbind(students_2020, students_2023)
#summary(students)
```
Note that before we combine the two data sets with `rbind()` ("row bind") we make a new variable called `Year` and we add year to the ID so that there are unique IDs for each person also in the combined data. We also check that the column names in the two data sets are identical. You should also check again that the summary looks as expected (commented out with "#" here).
...
Note that the data points appears to be clustered in many groups. This is because we have three values of each person and the only thing that separates them is measurement error (some groups also rounded off to closest cm, so some points are plotted on top of each other and appear as one). Ignoring this fact would clearly be wrong as we would think have three times as many individuals in the data as we actually have (this would be a case of "pseudoreplication" that will get back to later in the course).
...
The stippled lines in the plot shows the 95% *confidence intervals* for the prediction lines. Remember that this confidence interval represent the uncertainty in the mean height among *all* people with the given sex and foot length in the population. The more data we have, the narrower this confidence interval will be. You could also try to plot the prediction interval (see above) - this interval will not become narrower with more data. Take a moment to study this plot. What interpretations can you make?
...
We can also use the code above for plotting the predictions (note that only the two first lines need to be changed):
...
Take a moment to study this plot and the model summary above, and compare this to the result from the additive model. Note that in this model, $\beta_1$ is the difference between the two sexes *only at the intercept* (when foot length is zero) as we no longer assume that the two prediction lines are parallel. Hence, $\beta_1$ has no direct biological interpretation (as nobody has foot length of zero) 

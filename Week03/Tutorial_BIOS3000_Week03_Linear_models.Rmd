...
In this week's tutorial we will get introduced to linear statistical models, and analyze the morphometric data we collected in last week's group work. I suggest you run the code below by copy-pasting into an empty R script or Quarto file.

I also suggest you create sub-directories "Week01", "Week02", "Week03" and so on somewhere on your hard-drive. Then download all the data-files (from this year and previous years) to the "Week03" folder. Note that the data file must be in the same folder as the Quarto (qmd-file) that will read it.

<!-- For 2022: -->
<!-- ## Loading data -->

<!-- In 2020, the BIOS3000/4000 students took three repeated measurements of their own height, foot length, neck circumference, etc. This was not practical to do this year due to the COVID pandemic. Hence, we will in this tutorial use the data from last year's students. The data file "student_means.csv" contains the means of the three repeated measurements of each student. This is a plain text file with values separated by a comma ('csv' stands for 'comma separated values'). Start by reading the data into R and look at the first 6 rows of data: -->
...
I have combined all data the the 6 student groups this year in a single tab-delimited text file called "MeasurementData_2026.txt". This files can be read with the `read.delim` function using default arguments (see `?read.delim`):

```{r}
students_2026 = read.delim("MeasurementData_2026.txt", stringsAsFactors = TRUE)
```

Look at a summary of the data:

```{r}
summary(students_2026)
```

This shows a summary of the distribution of each of the variables in the data. When importing data, it is important to check the summary to make sure that there are no unexpected NA's (missing values) or inconsistent labeling such as "male" and "Male" (I had to fix some issues of this type).

Since there are rather few students this year, we will combine the data with the data from previous years (there were no measurements in 2021 and 2022 due to COVID and the course was not offered in 2024):
...
students_2025 = read.delim("MeasurementData_2025.txt", stringsAsFactors = TRUE)
students_2020$Year = "year_2020"
students_2023$Year = "year_2023"
students_2025$Year = "year_2025"
students_2026$Year = "year_2026"
students_2020$ID = paste(students_2020$ID, "2020", sep="_")
students_2023$ID = paste(students_2023$ID, "2023", sep="_")
students_2025$ID = paste(students_2025$ID, "2025", sep="_")
students_2026$ID = paste(students_2026$ID, "2026", sep="_")

# all(names(students_2026) == names(students_2025)) # Checking that column names are identical
students = rbind(students_2020, students_2023, students_2025, students_2026)
# summary(students)
# save(students, file = "students.RData")
```
Note that before we combine the three data sets with `rbind()` ("row bind") we make a new variable called `Year` and we add year to the ID so that there are unique IDs for each person also in the combined data. We also check that the column names in the data sets are identical. You should also check again that the summary looks as expected (commented out with "#" here).
...
Here there are three measurement for each individual. Ignoring this fact would clearly be wrong as we would think that we have three times as many individuals in the data as we actually have (this would be a case of "pseudoreplication" that will get back to later in the course).

In this tutorial, we will mainly work with the means for each person. We will also ignore the categorical variables `Eye_colour` and `Hair_colour`. To compute the means of the numerical measurements for each person, we can use the `aggregate()` function. We do this here and use the `head()` to see the first lines in the new data frame:
...
The first argument in the `aggregate()` function specifies what should be aggregated (we use columns 6 to 13 in `students`), the second argument is a list of grouping variables (we only use one here), and the last argument specifies what function should be applied to each group. The argument `na.rm = TRUE` specifies that missing values should be removed before the function is applied to each group.

Now we have a new data frame `student_means` that only has one value for each person (the mean of the three measurements). We have however lost the variables `Sex` and `Year`. A trick to retain these variables in the new data frame is to include them as a grouping variables (although it will not refine the groupings because each person only has one sex and occurs in only one year). Below, we also set the names of the three first variables (the grouping variables).

```{r}
student_means = aggregate(students[,6:13], by=list(students$ID, students$Sex, students$Year), FUN=mean, na.rm = TRUE)
names(student_means)[1:3] = c("ID", "Sex", "Year")
head(student_means)
```
...
We want to use these aggregated data in later tutorials, so let's save it to your working directory (we here create a text file with "comma separated values" (csv)):
...
Here we see that those with longer feet tend to be taller than the ones with shorter feet. We can look closer at this relationship by adding a regression line to the plot. Below, we also add a red horizontal line at the mean of `Height` (if you do this in a normal R script instead of in a Quarto document, you don't have to create the plot again)
...
This hierarchical structure may seem a bit cumbersome here, but this is a tidy and useful way to formulate more complex models that we get to later in the course. 
...
# or just (based on the hierarchical formulation above)
...
We have now estimated that the mean height of females in the population is `r sprintf("%.1f", means[1])` cm, and for males the mean height is `r sprintf("%.1f", means[2])` cm. The standard deviation describing the variation among females and males are respectively `r sprintf("%.1f", SDs[1])` cm and  `r sprintf("%.1f", SDs[2])` cm. These estimates are associated with some uncertainty. You may remember from last week that the standard error of the mean is the standard deviation divided by $\sqrt{n}$ (where $n$ is sample size). Using this we get:

```{r}
(SEs = SDs/sqrt(table(student_means$Sex)))
```

Last week we showed that the mean computed from independent samples from *any* population is approximately normally distributed. In a normal distribution, the 2.5 percentile is approximately 2 standard deviations below the mean, and the 97.5 percentile is approximately 2 standard deviations above the mean. Hence, the 95% confidence interval is approximately $\pm$ 2 SE's around the mean (remember that SE is the standard deviation describing the uncertainty of the mean (or any parameter)). Hence, the 95% confidence interval for mean height of females is
...
In 2005 the mean height of men entering the mandatory military service in southern Norway was 180.2 cm, which is within the confidence interval we have estimated here (see https://www.ssb.no/a/publikasjoner/pdf/sa94/del-v-1b.pdf).
...
We can confirm that the mean and standard errors are exactly the same as computed above. The confidence limits deviate slightly due to different approximating methods.

## Expanding the linear model (with alternative parameterizations)
...
You may be surprised that the two fixed effects parameters (i.e., the two "coefficients" in the summary) are not mean height for each of the sexes. This is because the model that we have fitted is
$$
...
The first parameter, $\beta_0$, is called the "intercept" and is the value of $y_i$ that we get if we set all the predictor variables (in this case only $x$) to zero. The only reason why the intercept is defined as the mean of females, and not males, is that "female" is earlier in the alphabet than "male" (factor levels are sorted alphabetically by default, but this can be changed).
...
res_sd = summary(fit_Sex)$sigma # Estimated residual standard deviation
...
This shows that a 95% confidence interval for the residual standard deviation in this model range from `r sprintf("%.1f", CI_sd[1])` cm to `r  sprintf("%.1f", CI_sd[2])` cm.
...
This model gives predictions of a person's height based on the persons sex and length of the foot, and can be formulated as 
...
From the summary, we see that the slope ($\beta_2$) is estimated to be `r round(coef(fit_Sex_Foot)["Foot"], 4)`. This means that predicted Height increases with `r round(coef(fit_Sex_Foot)["Foot"], 2)` cm for every cm increase in foot length. In addition, males are on average `r round(coef(fit_Sex_Foot)["Sexmale"], 2)` cm taller than females with the same foot length. Note that the effect of `Sex`, $\beta_1$, is now much smaller than the difference between the sexes that we estimated earlier when we did not include `Foot` in the model. This is because males tend to have longer feet than females, and hence some of the sex difference in height is explained by the effect of foot length. To test your understanding, it is a good idea to make a sketch of what you think the model predictions will look like (draw a line for each sex in a graph where you have 'Foot length' on the x-axis and 'Height' on the y-axis) - then you can compare it to the plot further down in the document (don't scroll yet!). 

But what does the intercept ($\beta_0$) in this model tell us? If we set $x_{i,1}$ and $x_{i,2}$ to zero, we get $\hat{y}_i = \beta_0$. Hence, the intercept ($\beta_0$) is the predicted height of a female with foot length equal to zero cm. This parameter is not directly interpretable. If you want this parameter to be directly interpretable, you could subtract the mean foot length of females from $x_{i,2}$ and instead fit the model

$$
y_i = \beta_0' + \beta_1 x_{i,1} + \beta_2 (x_{i,2} -\mu_F) + \varepsilon_i, \; \;\;\;\;\; \varepsilon_i \sim N(0, \sigma)
$$

where $\mu_F$ is the mean foot length of females. The intercept $\beta_0'$ is now the mean height of females with foot length equal to the mean in the sample. The other parameters remain the same. This is a *reparameterization* of the original model, meaning that the predictions and residuals from the model will remain the same (the model is really the same, it is just described in a different way). Note that  $\beta_0 = \beta_0' - \beta_2 \mu_F$.
...
plot(Height ~ Foot,
     ...
     xlab="Foot length (cm)",
     ylab = "Height (cm)",
     col = ifelse(Sex=="female", "red", "blue")
)
lines(fit ~ Foot, data = pred, subset = Sex=="female", col="red")
lines(lwr ~ Foot, data = pred, subset = Sex=="female", col="red", lty=2)
lines(upr ~ Foot, data = pred, subset = Sex=="female", col="red", lty=2)
lines(fit ~ Foot, data = pred, subset = Sex=="male", col="blue")
lines(lwr ~ Foot, data = pred, subset = Sex=="male", col="blue", lty=2)
lines(upr ~ Foot, data = pred, subset = Sex=="male", col="blue", lty=2)
legend("topleft", legend=c("Females", "Males"), pch=c(1,1), col = c("red", "blue"))
title(Title)
```

The stippled lines in the plot show the 95% *confidence intervals* for the prediction lines. Remember that these confidence intervals represent the uncertainty in the *mean* height among all people with the given sex and foot length in the *population*. The more data we have, the narrower this confidence interval will be. Take a moment to study this plot. What interpretations can you make?

*The fact that the two prediction lines are parallel is not a result of the analysis -- it is a constraint in the model!*. To allow different slopes for the to sexes, we need *interaction effects* (next section).

## Interaction effects

To make the model more flexible such that the two sexes are allowed to have different slopes, we need to add one parameter to the model. Again, there are several ways to parameterize this model. We will stick to the default parameterization in R and fit the following model:
...
We can also use the code above for plotting the predictions (only the two first lines need to be changed):


```{r echo=FALSE}
# This chunk is not printed because I have used echo = FALSE. Below, I have made the plot in a slightly different (but equivalent) way than above. In earlier versions of R, the 'data' and 'subset' arguments were not available in the 'plot' function. Hence, the 'with' function was convenient. There are often many ways to do the same thing in R and which way to use is partly a matter of taste - pick a way that you understand and is tidy and transparent (easier to avoid mistakes)
...
The difference in the slopes of the two sexes ($\beta_3$, `Sexmale:Foot` in the summary) is estimated to be `r round(coef(fit_Sex_Foot_interaction)["Sexmale:Foot"], 4)`, This means that the slope for males is `r -round(coef(fit_Sex_Foot_interaction)["Sexmale:Foot"], 2)` cm lower than the slope for females. That is, while the slope for females is `r round(coef(fit_Sex_Foot_interaction)["Foot"], 2)` cm ($\beta_2$, `Foot` in the summary), the slope for males is `r round(sum(coef(fit_Sex_Foot_interaction)[3:4]), 2)` cm ($\beta_2 + \beta_3$). This is not a big difference and hence the lines appear almost parallel in the plot.

Note that in this model, $\beta_1$ (`Sexmale` in the summary) is the difference between the two sexes *only at the intercept* (when foot length is zero) as we no longer assume that the two prediction lines are parallel. Hence, $\beta_1$ has no direct biological interpretation (as nobody has foot length of zero). 

You may have noticed that the confidence intervals for the predictions are slightly wider for the more complex interaction model (particularly for males, where there are fewer data points). This is a general phenomenon; If we add more parameters to a model, we reduce bias (increase accuracy) but we also loose precision (get wider confidence intervals). The summary of the model shows that the difference between the slopes of the two sexes is not statistically significant at the 5% level, meaning that the 95% confidence interval will overlap with zero. This justifies preferring the simpler additive model **if** we are only interested in using the model for predicting a person's height from the person's sex and foot length (we will look closer at such data-based model selection next week). However, it is important to realize that this **does not** justify a conclusion that females and males have the same slope. Only the confidence interval for the difference in slope provides information about how different the slopes may be.
...
3. What is the intercept for each of the models and what do their values mean? [remember that the intercept is the value of the response variable when all predictor variables are set to zero]. What do the values of the other parameters mean? 

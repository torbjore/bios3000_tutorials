...
We may also replace the vector $\mathbf{x}$ above with the whole design matrix $\mathbf{X}$. We then get the variance-covariance matrix of the predictions. The diagonal of this matrix holds the variances of the predictions (the square-root of these are the standard errors of the predictions), and the off-diagoal elements are covariances.

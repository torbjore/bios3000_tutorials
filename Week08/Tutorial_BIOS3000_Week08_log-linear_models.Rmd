---
title: Tutorial and Assignment Week 6 - Log-linear log-normal models
...
So far we have just distinguished between categorical (nominal) and numerical (continuous or discrete) variables. There are however some important subdivisions of these scale types, that we will introduce here:

* **Nominal** (= Categorical)**:**
Names or labels in no particular order. Examples: sex, species names, fertilizer type.

* **Ordinal:**
Names or categories in a particular order ranked from low to high. Examples: wind force on the Beaufort scale, age groups, categorical degrees of risk-prone behaviour.
...
Let's say that $x_1$ in the above expression is a binary indicator variable (e.g., 0 for females and 1 for males) and that $x_2$ is a continuous ratio-scaled variable (such as foot length). How will a change in these variables affect $y$? Changing $x_1$ from $0$ to $1$ means that $y$ gets multiplied by $e^{\beta_1}$. I.e., $e^{\beta_1}$ is the **relative difference** between e.g. males and females in $y$ (e.g. height) (remember that $e^0 = 1$).
...
In an additive log-linear model, predictions become parallel on the logarithmic scale instead of on the arithmetic scale as before. We'll give an example of this later.
...
The data collected by Capellini et al. (2010) was published as an online supplementary and can be read straight into R from the publication's web-site:

```{r}
Metabol = read.table("http://www.esapubs.org/archive/ecol/E091/198/data.txt?fbclid=IwAR1bHRp39-6bIYb4K62a5PdregQhpAWuVxdZwOyNOTJfPn7lAHVBVLKyrK0", sep = "\t",  header = TRUE, na.strings = "-9999")
# summary(Metabol)
...
We see the the power law exponent is estimated to be `r coef(fit)[2]` (95% confidence interval: `r confint(fit)[2,]`). These values are between the exponent of 2/3 = 0.67 (suggested by surface volume ratio of mammals) and 3/4 = 0.75 (["Kleiber's Law"](https://en.wikipedia.org/wiki/Kleiber%27s_law)). An exponent of `r coef(fit)[2]` means that a doubling of body mass is associated with multiplication in basal metabolic rate (BMR) by $2 ^ {`r round(coef(fit)["log(BodyMass)"], 4)`} = `r round(2^coef(fit)["log(BodyMass)"], 3)`$. 
...
We see that some bears have been captured and measured several times (variable `Obs.No`). To avoid any pseudoreplication (more on that next week), we will restrict our analysis to first observations. We also see that the two nominal variables `ID` and `Sex` have been given numerical values, so we change this to character strings as well:
...
These exercises and questions are are based on the three worked examples above and meant to help you understand the material in this tutorial and give you some training in working with log-linear log-normal models. It is *not* essential that you do them before you have a go at the assignment - if you think you already have a good understanding of these models, you can go straight to the assignment. We recommend that you do the exercises though. If you have problems answering some of the questions below, then you have a something to ask about at Help Desk! We will also try to go through these exercises during the Thursday lecture.
...
In this week's assignment you will estimate the allometric proportions of students with log-linear log-normal models (since all subjects are adults, this is referred to as "static allometry"). Instead of modelling students' height, you should now use body height as a predictor variable and pick one of the other variables as your response variable. The aim of your analysis is simple: Estimate the relative increase in this variable per relative increase in body length for both sexes. Express this allometric relationship as e.g. X% increase in Foot length per 10% increase in body length. First, fit a model with an additive sex effect and estimate the relative difference between males and females as well as the allometric relationship. Then, fit an interaction model and estimate a separate allometric relationship for each sex. All estimates should be associated with a 95% confidence interval (assuming that the sample is representative for some larger population such as all students at the university).

If you work together with other students, you should pick different variables to estimate the allometric relationship for.

To use all the data we have available, you should combine the "vitruvian" data (Bio-2150 students) you used during the R course of Week 1 with the mean measurements of yourself (you created the data file during Week 1). Start by reading in these data and look at their summary:

```{r}
bio2150 = read.table("../Week01/vitruvian.txt", header = TRUE)
bios3000 = read.csv("../Week03/students_means.csv")
summary(bio2150)
summary(bios3000)
```
...
The strategy we will use to combine the data is to first make the Bio-2150 data look like the Bios-3000 data, then stack the two data frames on top of each other by using the `rbind` function.

The `bio2150$gender` variable has been defined as a 'factor'. This means that we are not allowed to enter any new values (labels) into this vector than what has been defined as "levels" for the factor. We can tell that this variable is a factor by how it has been presented in the summary above; as a count of each factor level. We can also check this with `is.factor()` and we can see the defined levels by applying the `levels()` function:

```{r}
is.factor(bio2150$gender)
...
```{r}
levels(bio2150$gender) = c("female", "male") # NB! you have to be sure about the order of the levels (default is alphabetically)
```
...
Just like the `levels` function, the `names` function can also be used on the left side of an assignment to change the names of the variables. We'll do this here for the variables "gender", "body.length", "foot.length", and "neck.size" (if you are using any other response variable than foot length and neck circumference, you need to include them here too)
...
```{r}
biostudents = rbind(
  bio2150[,c("Sex", "Height", "Foot", "Neck_circumference")],
  bios3000[,c("Sex", "Height", "Foot", "Neck_circumference")]
)
summary(biostudents)
```

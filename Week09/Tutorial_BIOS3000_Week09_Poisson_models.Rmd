---
title: Tutorial and Assignment Week 8 - Log-linear Poisson models
...
The blue line is the one-to-one line (intercept = 1, slope = 1), so this looks wonderful!
...
We have already formulated the stochastic model we simulated data from in the previous section mathematically. Let's write it on the standard hierarchical format that we used in the Week 6 (and 3) tutorial, here also using the matrix formulation for the linear predictor with any number of predictor variables in the vector $\mathbf{x}_i$:
...
Now that you have learned about mixed effects models (Week 7 of the course), you probably have a feel for what this option is about.
...
where $\mathbf{x}_i \boldsymbol{\beta}$ is the fixed effects of the model, and the $\delta$s are the random effects. The indices $l(i)$ and $b(i)$ are the location and brood of chick $i$. Note that in Poisson models (and binomial models) we can have a variance component at the level of the observations (chicks in this case). In the models based on the normal distribution this variance component would be the same as the residual variance, and we can not have two parameters for the same thing. This model can be fitted with the `glmmTMB` function in the R package with the same name. You can use this function in much the same way as the 'lmer' function we used last week, only adding the argument `family = poisson`. For example, to fit the above model with interacting fixed effects of `YEAR`and `cALTITUDE`, we use `glmmTMB(TICKS ~ YEAR * cALTITUDE + (1|LOCATION/BROOD/INDEX), family=poisson, data=grouseticks)`. We will not do this here, but leave it as an *optional* part of the assignment and will include it in our suggested solution.
...
i. Can you say that these results indicate that there was a statistically significant decline or increase over time (the three years) in the density of ticks in this area? Explain. What are your interpretations of the patterns you see.

...
author: "Torbjørn Ergon, Tom Andersen, Yngvild Vindenes"
date: "April 2023"
output:
  html_document:
    toc: yes
...
The statistical basis for the MPN method was worked out by R. A. Fisher, and he used this as an example of maximum likelihood estimation. We will here use simulation to illustrate Fisher’s analysis of the method, and show how the MPN estimate can be found using a particular kind of generalized linear model.
...
<!-- Dette er basert på noen simuleringer jeg gjorde over som nå er tatt bort under revideringen. Jeg kommenterer dette derfor bort for nå. Uansett er dette litt vel avansert for dette kurset, og denne tutorialen blir lang nok so den er (med en del annet stoff som er "valgfritt") -->
...
We can construct a contingency table for the numbers

```{r}
SideEffects <- cbind(c(6,130000-6),c(30,2e7-30))
rownames(SideEffects) <- c("Yes", "No")
colnames(SideEffects) <- c("Norway", "Worldwide")
SETable <- as.table(SideEffects)
names(attributes(SETable)$dimnames) <- c("SideEffect","Group")
SETable
```

<br>

Probability of side effects in Norway and the World:
```{r}
(p_Norw <- SETable["Yes","Norway"] / sum(SETable[,"Norway"]))
(p_world <-SETable["Yes","Worldwide"] / sum(SETable[,"Worldwide"]))  
```

<br>

Corresponding odds of side effects:

```{r}
(odds_Norw <- SETable["Yes","Norway"] / SETable["No","Norway"]) 
(odds_world <-SETable["Yes","Worldwide"] / SETable["No","Worldwide"]) 
```

When p is very small, then the odds will be very similar to p (because we divide by a number that is only very slightly below 1).
...
Odds ratio:
```{r}
(OR <- odds_Norw/odds_world)
```
The odds of getting side effects are more than 30 times higher in Norway compared to the world.

<br>

Make a glm to obtain confidence intervals. The response variable will be the proportion of successes (side effects) and the proportion of failures in different groups (Norway or Worldwide). We need to create a data frame including the proportions (or numbers) as columns.

```{r}
#Construct data frame
Data <- data.frame(
    group = c("Norway", "World"),
    y = c(6, 30),
    ny = c(130000-6, 20*10^6-30)
)

fit <- glm(cbind(y,ny) ~ group, data=Data, family = binomial)
...
To find the odds ratio (for world to Norway) we take the exponent of the slope (the "world" effect), which represents the difference $\log(World)-\log(Norway)$.

```{r}
exp(coef(fit)[2])
```

The odds ratio for Norway compared to the world is the inverse:
...
#For Norway to World:
exp(-confint(fit)[2, 2:1]) 

#Or alternatively
1/exp(confint(fit)[2, 2:1])
```
...
<div class = "cornsilk">
```{r}
d <- read.csv("PimaDiabetes.csv")
d$diabetes <- as.factor(d$diabetes)

#Make 0/1 variable for diabetes
d$diabetes2 <- ifelse(d$diabetes=="Yes",1,0)
...
<div class = "cornsilk">
```{r}
# Leave out the response variable "diabetes" ("diabetes2")  
plot(d[,-c(8,9)],pch=19,cex=.2,col=4)

# Correlation matrix
round(cor(d[, -c(8,9)]), 3)
```

From this we see that the two predictor variables `bmi` and `skin`  are strongly correlated (r = 0.659) which is not unexpected as both are measures of obesity. In addition, `npreg` and `age` also strongly correlated (r = 0.667), also not surprising as the total number of pregnancies increases with age. We should suspect multi-collinearity (another word for dependence) between `bmi` and `skin`, and between `npreg` and `age`, and  therefore avoid using these variable pairs together in the regression model.
...
C. Use boxplots to investigate the distributions of the predictor variables grouped by diabetes status. You can use the `boxplot()` function with a formula like `glu ~ factor(diabetes)`. 


<div class="purple">
Tip (optional): You can also use ggplot syntax to make the plot, as in the assignment for week 7 and 8. In that case, it is a good idea to convert the dataframe to long format first.  To do this you can use 
...
```{r}
par(mfrow=c(3,3))
boxplot(npreg ~ diabetes, data=d)
boxplot(glu ~ diabetes, data=d)
boxplot(bp ~ diabetes, data=d)
boxplot(skin ~ diabetes, data=d)
boxplot(bmi ~ diabetes, data=d)
boxplot(ped ~ diabetes, data=d)
boxplot(age ~ diabetes, data=d)

```

Second option (ggplot syntax):
```{r,message=FALSE, warning=FALSE}
library(tidyverse)
d_longformat <- d %>% 
  pivot_longer(c(-diabetes),
               names_to = "Predictor", values_to = "Value")


ggplot(d_longformat, aes(x=diabetes,y=Value)) +
  geom_boxplot(aes(fill=Predictor),alpha=.5) +
   theme_bw()+
  facet_wrap(vars(Predictor),scales="free_y")+
   labs(x="Diabetes", y="Value") +
  theme(legend.position = "none", axis.text.x = element_text(angle = 45,hjust=1))

```
 
The glucose tolerance test (`glu`) shows the largest difference between diabetic and non-diabetic individuals, which is perhaps not surprising since this test is a key element in the diabetes diagnosis. It is also the only predictor variable that requires special equipment and medical competence.
...
<div class="cornsilk">
A logit link glm model for diabetes risk, excluding the predictors `npreg` which is correlated with `age`, and `skin` which is correlated with `bmi`. 

```{r}
summary(m <- glm(diabetes ~ ped+bmi+glu+bp+age, data=d, family=binomial))
```


`(Intercept)` is the log(odds) of diabetes for a person with all predictor variables set to 0, which is somehow totally irrelevant. If we had centered the predictor variables (e.g. `bmi - mean(bmi)` etc) the intercept would have represented the log(odds) of an average person, which certainly seems more biologically relevant! If we take `bmi` as an example, log(odds) of diabetes increases with 0.086 $\pm$ 0.022 units per unit increase in `bmi`.

We can back-transform to look at the odds of diabetes for each predictor:
```{r}
round(exp(coef(m)),3)
```
...
<br>
Looking at `bmi`, this indicates a 1.09-fold increase in odds of diabetes per  `bmi` unit.
...
The 95% confidence interval for the slope of bmi indicates that the odds of developing diabetes increases 1.04 to 1.14 times with a unit increase in `bmi`. In other words, obesity has a strong effect on diabetes risk.
...
We choose to make predictions for `bmi`-values ranging from 20 to 70. For this, we will call `predict` but this time with `newdata` as a data frame with a single variable named the same as the predictor in the model (`bmi`). Since we want to construct confidence limits, we ask for predictions on the default logit link scale and with standard errors (`se.fit=TRUE`). These are then used to make prediction and confidence limits on the logit scale, which are back-transformed to the original scale using the formula $1/(1+\exp(-\alpha)$.

Finally the result is plotted (here using ggplot syntax).

```{r}
#Predict for BMI
predBMI <-  predict(m, newdata=data.frame(glu=mean(d$glu),bp=mean(d$bp),bmi=20:70,ped=mean(d$ped),age=mean(d$age)), se.fit=TRUE, type="link")

#Data frame to store prediction
predframe <- data.frame(bmi=20:70)
predframe$predicted <- 1/(1+exp(-predBMI$fit))
predframe$lower <- 1/(1+exp(-(predBMI$fit-1.96*predBMI$se.fit)))
predframe$upper <- 1/(1+exp(-(predBMI$fit+1.96*predBMI$se.fit)))

 
ggplot(d, aes(x = bmi, y = diabetes2)) +
      geom_point(alpha = 0.5) +
      theme_bw() +
    geom_line(data=predframe,aes(x=bmi,y=predicted), color=4,size=1.2)+  
  geom_ribbon(data=predframe,aes(x=bmi,y=predicted,ymin = lower, ymax=upper), fill=4,alpha=.3,color=NA) + 
    labs(x="BMI", y="P(Diabetes)")+
...
We see that the `bmi` distribution is quite skewed with few individuals having `bmi` > 50 (BMI > 40 is often considered "extreme obesity"), so it could seem that our model is quite heavily influenced by a few extreme cases. 

Alternatively, with base R syntax we can use the `rug` function to draw short vertical lines representing the actual `bmi` measurements that the model was based on. 
...
```{r}
plot(predframe$bmi, predframe$predicted, type="l", lwd=3, col="red", 
     ylim=c(0,1), xlab="Body Mass Index (BMI)", ylab="Pr(diabetes)")
lines(predframe$bmi, predframe$upper, lty=2, col="red")
lines(predframe$bmi, predframe$lower, lty=2, col="red")
abline(h=0.5, lty=2)
rug(d$bmi)
```
</div>
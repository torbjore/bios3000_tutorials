...
You can read the "$\sim$" as "drawn from". This means that $P(Y=y) = \binom{n}{y} p^{y} (1-p)^{n-y}$, where $Y$ is the the stochastic variable representing the number of successes you may get in $n$ trials (the notation $P(Y=y)$ means "the probability that $Y$ takes the value $y$"). The parameter $p$ is the probability of success (e.g. testing positive for the disease) for a single study unit (e.g., individual) and study units are assumed to be independent. The expression to the right of the equal sign is the expression for the binomial probability distribution (often called "probability mass function" since the observations ($y$) can only be whole numbers (counts)).
...
We notice in passing that the probabilities for success and non-success in the same treatment have the same denominator (`colSums(az`)) such that we can also calculate the odds directly from the counts (`az["Infected", ] / ac["Not.Infect", ]`); check it out and take moment to think about why this has to be so. Finally, we can calculate the odds ratio of the treatments, which represents the relative change in odds between the treatment and the placebo:
...
In other words, we have 95% confidence that the odds of infection could be from 52% lower to 71% higher in the vaccine treatment relative to the placebo. Since the interval encloses equal odds (1:1), we find little support for claiming that the AstraZeneca-Oxford vaccine prevents infection by B.1.351 variant of the SARS-CoV-2 virus.
...
B. Our task is to predict `diabetes` from other variables. It is therefore a good idea to first look at relationships between the independent variables by making scatterplot and correlation matrices (hint: `plot()` on a `data.frame` object is identical to using the `pairs()` function). *Are there any independent variables that are closely related? If so, can you think of explanations for this?* (hint: look at for example `bmi` vs. `skin`, or `age` vs. `npreg`). *Which considerations should we make when using any of these pairs in a model?*

C. Use boxplots to investigate the distributions of at least 4 independent variables grouped by diabetes status. You can use the `boxplot()` function with a formula like `glu ~ factor(diabetes)`. Notice that this works better if you cast the now logical variable `diabetes` into a factor again. Using the argument `horizontal=TRUE` in the `boxplot()` call makes the figure easier to read.

D. Pick one of the independent variables you investigated in part C and make a univariate binomial `glm` with logit link where you use this variable to predict diabetes status. Notice that since our dependent variable is binary, we do not need to use `cbind()` to make a 2-column vector for the dependent variable. *Explain how you interpret the intercept and slope estimates in this model.* *Make a 95% confidence interval for the relative change in odds per unit change in the independent variable* (depending on the variable you choose, it may increase readability to compute the relative change per 10 or 100 units).
